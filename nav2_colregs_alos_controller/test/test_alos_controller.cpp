...
public:
  ...
  void setResetState(bool enabled, double initial_beta, double current_beta)
  {
    reset_beta_on_new_goal_ = enabled;
    beta_hat0_ = initial_beta;
    beta_hat_ = current_beta;
  }

  double betaHat() const
  {
    return beta_hat_;
  }
};
...
TEST(ALOSControllerGoalReset, SetPlanResetsBetaOnlyForNewGoal)
{
  TestableALOSController controller;
  controller.setResetState(true, 0.1, 0.4);

  controller.setPlan(makePath("map", 10.0, 2.0));
  EXPECT_DOUBLE_EQ(controller.betaHat(), 0.4);
  controller.setPlan(makePath("map", 11.0, 2.0));
  EXPECT_DOUBLE_EQ(controller.betaHat(), 0.1);
}

TEST(ALOSControllerGoalReset, DisabledResetPreservesBetaForNewGoal)
{
  TestableALOSController controller;
  controller.setResetState(false, 0.1, 0.4);
