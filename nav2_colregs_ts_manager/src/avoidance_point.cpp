...
: rclcpp::Node("avoidance_point_node")
{
  declare_parameter("os_radius", 5.0,
    parameterDescription("Physical own-ship collision radius [m].", false));
  ...
  declare_parameter("point_extension_distance", 20.0,
    parameterDescription("Point range beyond current OS-TS distance [m], independent of inflation.",
      false));
  validateNumber("os_radius", get_parameter("os_radius").as_double());
  validateSafetyFactor(get_parameter("safety_factor").as_double());
    ...
    [](const std::vector<rclcpp::Parameter> & parameters) {
      ...
      try {
        for (const auto & parameter : parameters) {
          if (parameter.get_name() == "safety_factor") {
            validateSafetyFactor(parameter.as_double());
          } else if (parameter.get_name() == "os_radius" ||
            parameter.get_name() == "point_extension_distance")
          {
  ...
  RCLCPP_INFO(get_logger(),
    "Avoidance effective parameters: os_radius=%.3f radius_inflation=%.3f point_extension=%.3fm",
    get_parameter("os_radius").as_double(), get_parameter("safety_factor").as_double(),
  ...
  const std::shared_ptr<nav2_colregs_msgs::srv::GetAvoidancePoint::Response> response)
{
  ...
  if (!validSnapshot(state) || !fresh(state.header.stamp, now(), state_timeout_)) {
  ...
  if (std::hypot(os_x - state.os_pose.position.x,
    os_y - state.os_pose.position.y) > max_pose_delta_)
  {
    fail(Response::INVALID_REQUEST, "Request is not anchored near the snapshot OS pose");
    return;
  }
  const auto parameters = get_parameters({"os_radius", "safety_factor", "point_extension_distance"});
  const double os_radius = parameters[0].as_double();
  const double safety_factor = parameters[1].as_double();
  const double extension = parameters[2].as_double();
  int idx = selectPrimary(state, os_x, os_y, os_radius, safety_factor);
  if (idx < 0) {
  ...
  bool found = findSafeHeading(state, request->avoid_direction,
                               goal_x, goal_y, os_x, os_y, os_radius, safety_factor, safe_heading);
  if (!found) {
  ...
  arrow.lifetime = rclcpp::Duration::from_seconds(state_timeout_);
  ...
  double os_radius, double safety_factor)
{
  ...
  for (size_t i = 0; i < list.ships.size(); ++i) {
    ...
    const bool overlap = std::hypot(ts.pose.position.x - ox, ts.pose.position.y - oy) <=
      safety_factor * (os_radius + ts.radius);
  ...
  double & safe_heading)
{
  ...
  for (int i = 0; i < 180; ++i) {
    ...
    if (headingSafe(state, os_x, os_y, candidate, os_radius, safety_factor)) {
